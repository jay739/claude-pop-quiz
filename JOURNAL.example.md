...
- **Answer:** True but incomplete — the question was the *cost*. Every index must be updated on each write (slower `INSERT`/`UPDATE`/`DELETE`) and consumes storage, so you index the columns you actually filter/join/sort on, not all of them.
