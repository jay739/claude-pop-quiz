...
Agentic coding lets you ship work without reading what the agent wrote. The risk: you end up unable to _explain_ code, decisions, and tooling that are nominally "yours", whether in an interview, a review, or just when the agent isn't around. This hook counters that by **pop-quizzing you on your own sessions, on a cadence you can't skip.**

Every ~40 to 45 _actions_ in a chat (your messages **plus** every tool/file/agent call Claude makes), it pauses and makes you answer 5 questions about the technical substance of that session: the jargon used, the code written and _why_, and the files touched. You answer in your own words; Claude grades you and flags gaps.
...
- **Leitner spaced repetition.** Every topic's verdict history folds into a box: a correct answer promotes it, a miss resets it, a partial holds. The next quiz pulls your least-mastered topics and retires the ones you've nailed, so weak areas keep coming back until they stick.
...
- **Streaks and accuracy.** A correct-answer streak and lifetime accuracy show up in the quiz intro and in `status` to keep you going.
...
- **Randomized threshold** (default 40 to 45) so the cadence isn't gameable.
...
The hook **checks itself for updates**, so there is no need to remember to `git pull`.
...
- Put the hook in `~/.claude/settings.json` (user scope) and it fires in _every_ project.
- Put it in a project's `.claude/settings.json` and it fires only in that project.
...
| `POP_QUIZ_FORMAT`              | `essay`                                  | `essay`, `mcq`, or `mixed`; see [Quiz format](#quiz-format-and-cant-i-just-skip-it)              |
...
| `mcq`               | Every question is **multiple choice** (A to D, one correct). You answer in a single line of letters, e.g. `1C 2A 3D 4B 5A`. Seconds, no typing. |
...
The real lever is making the quiz cheap (MCQ takes seconds) so answering beats
...
```
# .pop-quiz-topics, for a Rust async project
...
The `PreToolUse` hook records every tool name and the files it touches into the
...
- **Copy `~/.claude/` to the new machine.** Hooks, state, and journal come with it
  and just work.
- **Clone this repo + run `./install.sh`** on each device. This installs the hook and
...
an action-count cadence (every ~40 to 45 actions) across _every_ chat, rather than a
...
MIT (c) 2026 Jayakrishna Konda, see [LICENSE](LICENSE).
